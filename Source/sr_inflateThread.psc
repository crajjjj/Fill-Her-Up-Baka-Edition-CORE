...
Function doInflation(Actor p, bool isVaginalOrAnal, float currentInf)
	If currentInf <= 0
		currentInf = 0
	EndIf
	if config.BodyMorph 
...
;	endif
	...
	if(inflater.config.bellyScale)
		while(currentInflation < inflationTarget)
			...
			tick -= 0.2
			if(tick <= 0)
				doInflation(akActor, true, currentInflation)
				tick = inflationTick
		...
		EndWhile
		if !BodyMorphApplied
			doInflation(akActor, true, inflationTarget)
		Endif
		...
		if !BodyMorphApplied
			doInflation(akActor, false, oralCum)
		endif
	EndIf
...
;	endif
	...
	if deflateTarget < maxInflation
		...
		If inflater.config.bellyScale
			...
			if !BodyMorphApplied
				doInflation(akActor, (isAnal || isVaginal), deflateTarget)
			Endif
		Else
			Utility.wait(tme)
		EndIf
